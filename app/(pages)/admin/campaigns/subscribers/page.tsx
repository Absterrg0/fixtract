...
export default function MarketingSubscribersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await authFetch(`${apiBase()}/api/admin/marketing-subscribers?${params.toString()}`);
      ...
      toast.error(message(error, "Failed to load subscribers"));
    } finally {
      setLoading(false);
    }
  }, [filters, page]);
  ...
  useEffect(() => {
    ...
    authFetch(`${apiBase()}/api/admin/marketing-campaigns/service-options`, { signal: controller.signal })
      .then(async (response) => {
        const json = await response.json().catch(() => null);
  ...
  const sync = async () => {
    setSyncing(true);
    try {
      const response = await authFetch(`${apiBase()}/api/admin/marketing-subscribers/sync`, { method: "POST" });
      const json = await response.json().catch(() => null);
      ...
      await load();
    } catch (error) { toast.error(message(error, "Subscriber sync failed")); }
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6">
    ...
    <Card><CardContent className="grid gap-3 p-4 md:grid-cols-[1.6fr_repeat(4,1fr)]">
      <div className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" placeholder="Search email" value={filters.q} onChange={(event) => setFilters((current) => ({ ...current, q: event.target.value }))} /></div>
      <Select value={filters.country} onValueChange={(country) => setFilters((current) => ({ ...current, country }))}><SelectTrigger><SelectValue placeholder="Country" /></SelectTrigger><SelectContent><SelectItem value="all">All countries</SelectItem>{countryOptions.map((country) => <SelectItem key={country.value} value={country.value}>{country.label}</SelectItem>)}</SelectContent></Select>
      <Select value={filters.locale} onValueChange={(locale) => setFilters((current) => ({ ...current, locale }))}><SelectTrigger><SelectValue placeholder="Language" /></SelectTrigger><SelectContent><SelectItem value="all">All languages</SelectItem>{LANGUAGES.map((locale) => <SelectItem key={locale} value={locale}>{locale.toUpperCase()}</SelectItem>)}</SelectContent></Select>
      <Select value={filters.service} onValueChange={(service) => setFilters((current) => ({ ...current, service }))}><SelectTrigger><SelectValue placeholder="Service" /></SelectTrigger><SelectContent><SelectItem value="all">All services</SelectItem>{services.map((service) => <SelectItem key={service.key} value={service.key}>{service.label}</SelectItem>)}</SelectContent></Select>
      <Select value={filters.status} onValueChange={(status) => setFilters((current) => ({ ...current, status }))}><SelectTrigger><SelectValue placeholder="Status" /></SelectTrigger><SelectContent><SelectItem value="all">All statuses</SelectItem><SelectItem value="active">Active</SelectItem><SelectItem value="unsubscribed">Unsubscribed</SelectItem></SelectContent></Select>
    </CardContent></Card>
    <Card><CardHeader><CardTitle>Subscribers <span className="text-base font-normal text-muted-foreground">({total})</span></CardTitle><CardDescription>Use filters to inspect the exact consent audience available to campaigns.</CardDescription></CardHeader><CardContent className="p-0">
      {loading ? <div className="space-y-3 p-6"><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /></div> : rows.length === 0 ? <p className="p-6 text-sm text-muted-foreground">No subscribers match these filters.</p> : <Table><TableHeader><TableRow><TableHead>Email</TableHead><TableHead>Name / role</TableHead><TableHead>Country</TableHead><TableHead>Language</TableHead><TableHead>Services</TableHead><TableHead>Status</TableHead><TableHead>Subscribed</TableHead></TableRow></TableHeader><TableBody>{rows.map((row) => { const servicesForRow = row.serviceKeys?.length ? row.serviceKeys : row.interestedServices || []; return <TableRow key={row._id}><TableCell className="font-medium">{row.email}</TableCell><TableCell>{row.firstName || row.name || "—"}<div className="text-xs text-muted-foreground">{row.role || "—"}</div></TableCell><TableCell>{row.region || "—"}</TableCell><TableCell>{(row.locale || "en").toUpperCase()}</TableCell><TableCell className="max-w-56">{servicesForRow.length ? servicesForRow.join(", ") : "—"}</TableCell><TableCell><Badge variant={row.unsubscribedAt ? "destructive" : "secondary"}>{row.unsubscribedAt ? "Unsubscribed" : "Active"}</Badge></TableCell><TableCell className="whitespace-nowrap">{date(row.subscribedAt)}</TableCell></TableRow>; })}</TableBody></Table>}
    </CardContent></Card>
